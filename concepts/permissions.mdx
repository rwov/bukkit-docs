...
<Warning>
  Permissions registered via `addPermission` exist only for the current server session. They are not persisted to disk and will be re-registered each time your plugin enables. If you change a permission's `PermissionDefault` at runtime using `Permission.setDefault()`, call `Permission.recalculatePermissibles()` afterward so all online players' effective permissions are updated immediately.
</Warning>
