...
Use SQLite, MySQL, PostgreSQL, pools, migrations, and async repositories.
...
CompletableFuture
    ...
    .thenAccept(profile -> Bukkit.getScheduler().runTask(
        plugin, () -> sessions.open(playerId, profile)
    ))
    .exceptionally(error -> {
        plugin.getLogger().log(Level.SEVERE, "Profile load failed", error);
        return null;
    });
```
